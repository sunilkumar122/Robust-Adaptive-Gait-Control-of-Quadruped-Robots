...
% smc values
knee_smc_lamda=150;%100/140final 140(For lqr_smc_reactio=165)
knee_smc_new=80;%80/100final 80(For lqr_smc_reactio=80)
hip_smc_lamda=95;%45/45final 85(For lqr_smc_reactio=95)
hip_smc_new=45;%25/45final 25(For lqr_smc_reactio=35)

knee_smc_lamdas=100;%100/140final 140(For lqr_smc_reactio=165)
knee_smc_news=60;%80/100final 80(For lqr_smc_reactio=80)
hip_smc_lamdas=85;%45/45final 85(For lqr_smc_reactio=95)
hip_smc_news=25;%25/45final 25(For lqr_smc_reactio=35)
...
knee_pitch_derivs=180;%100
knee_pitch_ints=7500;%300
knee_pitch_props=1200;%400

hip_roll_derivs=280;
hip_roll_ints=180;
hip_roll_props=300;

hip_pitch_derivs=200;%200
hip_pitch_ints=1500;%150
hip_pitch_props=500;%350/400/450/500

filter_coefficent_pids=100;
...
elseif pathType == 3    % straight line
    pathT = [0 1 60]';%(time in sec to complete the path)[0 60]
    pathX = [0 0.01 10]';%(lenght in meter)%% [0 10 ]for lqr[0 5]for smc[0 10]
    pathY = [0 0 0]';
end
